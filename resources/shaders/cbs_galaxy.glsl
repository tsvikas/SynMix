...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Second Layer
    vec3 p2 = vec3(uvs / (fieldScale + sin(animTime * 0.11) * 0.2 + 0.2 + sin(animTime * 0.15) * 0.3 + 0.4), 1.5) +
              vec3(2., -1.3, -1.);
    ...
    fragColor = colorIntensity * (mix(freqs[3] - .3, 1., v) *
                                      vec4(1.5 * freqs[2] * t * t * t, 1.2 * freqs[1] * t * t, freqs[3] * t, 1.0) +
                                  c2 + starcolor * starBrightness);
}
