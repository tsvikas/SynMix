...
#define H(h, sn2)                                                                                                      \
    (cos(h * h / (3. + (2. + 1. * abs(st(1.5))) * sn2) +                                                               \
         (1. + 4. * sn2) * vec3(2.5 * sn2 * st(1.2), 1. + 2. * sn2, 4)) *                                              \
         .9 +                                                                                                          \
     .4)
    ...
    for (float i = 0.; i < raycastSteps; i++) {
        ...
        for (float j = 0.; j < 7. + sn2; j++) {
            ...
            for (float k = 0.; k < 4. - 2. * sn2; k++) {
                w = vec4(w.x * w.x - w.y * w.y - w.z * w.z - w.w * w.w, 2. * w.x * w.y, 2. * w.x * w.z,
                         2. * w.x * w.w) -
                    .35 * p * detailLevel;
            }
        }
        ...
        c += .6e-1 * colorIntensity * mix(vec3(1), H(M(sc) * colorVariation, sn2), .9) *
             exp(-i * i * stepsize * max(1e1, sn2 * 2e1));
    }
