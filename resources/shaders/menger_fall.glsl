...
vec3 getLight(in vec3 color, in vec3 normal, in vec3 dir) {
    ...
    return (diffuse * diffuseFactor) * (LightColor * color) + (diffuse2 * diffuseFactor) * (LightColor2 * color);
}
...
float DE(in vec3 z) {
    ...
    for (int n = 0; n < Iterations; n++) {
        ...
        if (z.z < zFactor0 * Offset.z * (scale - 1.0))
...
vec3 getNormal(in vec3 pos) {
    ...
    return normalize(
        vec3(DE(pos + e.yxx) - DE(pos - e.yxx), DE(pos + e.xyx) - DE(pos - e.xyx), DE(pos + e.xxy) - DE(pos - e.xxy)));
}
