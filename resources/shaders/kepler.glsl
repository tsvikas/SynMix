...
float mountains(vec2 p) { return ridgedNoise(p * 2.0, 4) * 0.6; }
...
vec3 starField(vec3 dir, float time) {
    ...
    vec4 rand = vec4(hash(floor(localRay.xy * dense) / dense), hash(floor(localRay.xy * dense) / dense + vec2(1.0)),
                     hash(floor(localRay.xy * dense) / dense + vec2(2.0)), 1.0);
...
void main() {
    ...
    vec4 texNoise = vec4(fbm(surfaceCoord * 2.0, 4), fbm(surfaceCoord * 1.0, 3), fbm(surfaceCoord * 4.0, 5), 1.0);
    ...
    vec3 terrainColor = mix(deepOceanColor, shallowOceanColor, smoothstep(0.0, 0.3, continentMask));
    ...
    vec3 originalStyleLand =
        max(vec3(0.0), vec3(0.13, 0.65, 0.01) * land) + max(vec3(0.0), vec3(0.8, 0.4, 0.01) * land2);
    ...
    vec3 sunRef = GetSunColorReflection(refNorm, sunDir) * 0.005 * hit * (1.0 - saturate(reflectionMask * 3.5)) *
                  (1.0 - texS.x) * refNoise;
    vec3 sunRef2 = GetSunColorReflection(refNorm, sunDir2) * 0.005 * hit * (1.0 - saturate(reflectionMask * 3.5)) *
                   (1.0 - texS.x) * refNoise;
